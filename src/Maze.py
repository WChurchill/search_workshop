...
class Maze:
	...
	def __init__(self, width, height):
		...
		for wall in wallOrder:
			if((self.__vertical(wall)) and ( (wall + 1) % self.__width != 0)): #second false when cell is rightmost
			...
			elif( not self.__vertical(wall) and ((wall // self.__width) // 2 != (self.__height-1))): #second is false when wall is bottom most
	...
	def __UpPassage(self, coords):
		return self.__walls((coords[1]-1)*2*self.__width + coords[0] + self.__width) and ( coords[1] != 0)
	# cell wall down = cell wall up + 2 * width (y != height-1)
	def __DownPassage(self, coords):
		return self.__walls((coords[1])*2*self.__width + coords[0] + self.__width) and ( coords[1] != self.__height - 1)
	# cell wall left = cell wall up + width - 1 (x != 0)
	def __LeftPassage(self, coords):
		return self.__walls(coords[1]*2*self.__width + coords[0] -1) and ( coords[0] != 0)
	# cell wall right = cell wall left + 1    (x !- width-1)
	def __RightPassage(self, coords):
		return self.__walls(coords[1]*2*self.__width + coords[0]) and ( coords[0] != self.__width - 1)
	...
	def getTerminalFunc(self):
		def terminalTest(coords):
			return (coords[0] == self.__width-1) and (coords[1] == self.__height-1)
...
m.print()
print ("All connected: ", m.allUnioned())
